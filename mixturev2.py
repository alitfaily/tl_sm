"Mix source models to obtain probabilities"
...
class MOE(SurrogateBasedApplication):
    ...
    def _initialize(self):
        ...
        declare("input_models", None, types=list, desc="Set of source models to act as experts")
    ...
    def train(self):
        ...
        self._experts = self.options["input_models"] # Ali T addition 
        if self.options["input_models"] is None: 
            self.existing_experts = False
        else: self.existing_experts = True
        print(self.existing_experts)
        if not ctest:
            ctest = ytest
        values = np.c_[x, y, c]
        ...
        if self.existing_experts:
            # we try to fit the clusters based on source model errors on training data 
            et = np.zeros((len(_yt[:]), len(self._experts)))
            #fitdata = np.array(len())
            for i, xtin in enumerate(xt):
                for j, model in enumerate(self._experts): 
                    et[i,j] =  np.absolute(model._predict_values(np.atleast_2d(xtin)) - _yt[i])
                    print('at', xtin,'error from model',j, 'is', et[i,j] )
                    print('predicted value is', model._predict_values(np.atleast_2d(xtin)), 'and yt is', _yt[i])
            print('total w with et', np.c_[xt, et])
            self.cluster.fit(np.c_[xt, et])
        else:
    ...
    def _split_data_to_clusters(self,  output_variances=False):
        """
        This method splits data into clusters
        ...
        yt = self._training_values[:, nx : nx + 1]
        ct = self._training_values[:, nx + 1 :]
        et = np.zeros((len(yt[:]), len(self._experts)))
            #fitdata = np.array(len())
        for i, xtin in enumerate(xt):
            for j, model in enumerate(self._experts): 
                et[i,j] = et[i,j] + np.absolute(model._predict_values(np.atleast_2d(xtin)) - yt[i])
        ...
        # sort trained_values for each cluster
        cluster_classifier = self.cluster.predict(np.c_[xt, et])
        clustered_values = self._cluster_values(np.c_[xt, yt], cluster_classifier) 
        clusters = clustered_values
        return clusters
    ...
    def _predict_hard_output(self, x, output_variances=False):
        ...
        probs = np.array(
                [self._proba_cluster_one_sample(row) for row in x]
            )
        ...
        model = {}
        #select which model is best suited for each cluster
        for i in range(self.n_clusters): #range(len(sort_cluster))
            index = self._select_source_model(clustered_values[i])           
            model[i] = self._experts[index]
            print('cluster', i)
            print('best source model is:', index)
        #print(x.shape)
        for i, xin in enumerate(x): #np.atleast_2d(x)
            #find which cluster x lies in:
            idx = sort_cluster[i]
            print('x is ', xin)
            print('it lies in cluster', idx)           
           
            if output_variances:
                predicted_values.append(model[idx].predict_variances(np.atleast_2d(xin))[0])
            else:
                print(idx)
                print(model[idx])
                print(np.atleast_2d(xin))
                #print(model[idx].predict_values(xin.reshape(-1,1))[0])
                predicted_values.append(model[idx].predict_values(np.atleast_2d(xin))[0])
    ...
    def _predict_smooth_output(self, x, distribs=None, output_variances=False):
        ...
        sort_proba = np.array(
                [self._proba_cluster_one_sample(row,distribs) for row in np.atleast_2d(x)]
            )
       
        proba_model = np.ones((len(sort_proba),len(self._experts))) # model probs in clusters will be calculated in GPops
        ...
        for i in range(len(sort_proba)):
            ...
            for j in range(len(self._experts)):
                if output_variances:
                    ...
                    expert_value = (
                        self._experts[j].predict_values(np.atleast_2d(x))[0]
                        * sort_proba[i] * proba_model[i][j]
                    )
    ...
    def _select_source_model(self, clustered_values):
        """
        Determine which source model is best suited to represent each cluster
        -----
        Return: index of best         
        """
        dim = self.ndim
        scores = {}
        test_values = np.array(clustered_values)
        print('clustered_values',clustered_values)
        for i, smodel in enumerate(self._experts):
            expected = test_values[:, dim]
            actual = smodel.predict_values(test_values[:, 0:dim]).reshape(-1)
        ...
        best_index = None
        best_score = None
        for index, rmse in scores.items():
            print('rmse',rmse)
            if best_score is None or rmse < best_score:
                best_index, best_score = index, rmse

        print("Best expert = {}".format(best_index))
        return best_index
    ...
    def _proba_cluster_one_sample(self, x, distribs=None):
        """
        ...
        x=np.atleast_2d(x)
        noise = 0
        error_noise = noise*np.ones((x.shape[0],len(self._experts)))

        probs_gmm = self.cluster.predict_proba(np.c_[x , error_noise ] )
        print('probs from gmm are ', probs_gmm)
        return probs.flatten()#probs_gmm.flatten()#
